...
$DIST_INDEX_BASEDIR = "$DIST_DIR/v3"

$INDEX_FILE = "$DIST_INDEX_BASEDIR/index.json"
$CHANNEL_INDEX_FILE = "$DIST_INDEX_BASEDIR/channel-$Channel.json"
...
function Invoke-SnapToolchain {
    ...
    if ($VersionString -match 'v((\d+\.\d+\.\d+)(?:\+([a-f0-9]+))?)') {
...
function Invoke-MergeIndex {
    ...
    $currentTargets | ForEach-Object {
        ...
        $componentIndexPath = switch ($Channel) {
            'bleeding' { "$DIST_INDEX_BASEDIR/bleeding" }
            'latest' { "$DIST_INDEX_BASEDIR/latest/$componentToolchainVersion" }
            'nightly' { "$DIST_INDEX_BASEDIR/nightly/$Script:DateNightly" }
        }
